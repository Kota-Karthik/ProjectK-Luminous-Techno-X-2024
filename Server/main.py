...
def optimize_energy_usage():
    ...
    print(f"Predicted Energy Consumption for Tomorrow: {tomorrow_consumption:.2f} kWh")
    
    tomorrow_solar = forecast_solar_energy(solar_data)
    print(f"Predicted Solar Energy Production for Tomorrow: {tomorrow_solar:.2f} kWh")
    
    remaining_consumption = max(0, tomorrow_consumption - tomorrow_solar)
    print(f"Remaining Consumption after Solar Energy: {remaining_consumption:.2f} kWh")
    ...
    print(f"Peak tariff hour:  {peak_tariff_hour}")
    print(f"Low tariff hour: {low_tariff_hours}")
    energy_savings = calculate_energy_savings(remaining_consumption, predicted_tariffs, low_tariff_hours, peak_tariff_hour)
    print(f"Energy savings by consuming during low tariff hours: ${energy_savings:.2f}")
    print(f"Total time elapsed during training: {total:.2f} seconds")
